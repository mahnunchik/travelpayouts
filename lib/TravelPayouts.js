...
class TravelPayouts {
   ...
   * @param  {string} segments.origin - origin IATA or string "City, Country (IATA)". IATA code is shown by uppercase letters
   * @param  {string} segments.destination - destination IATA or string "City, Country (IATA)". IATA code is shown by uppercase letters (for example: "Berlin, Germany (BER)");
   * @param  {date|string} segments.date - departure date, Date object or string in format yyyy-mm-dd
   * @param  {object} options
   * @param  {string} options.host - host's request (must be replaced by the address of your website where API will be used)
